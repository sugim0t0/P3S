...
def wait_signal_update(task, sig_id, delay):
    ...
    task.cpu.rest_delay_cycle = delay
...
def set_signal_update(task, dst_task, sig_id, delay):
    ...
    if not b_changed and dst_task.priority > task.priority:
        # delay is doubled (current_task -> dst_task -> current_task)
        task.cpu.rest_delay_cycle = (delay * 2)
    else:
        task.cpu.rest_delay_cycle = delay
...
class TransAppFinish(p3s.Trans):
    ...
    def update(self, current_cycle):
        ...
        self.proc.cpu.rest_delay_cycle = 3 # Task switch delay
